...
func TestFileSessionRequiresManifestSpace(t *testing.T) {
	...
	sess := New(graphsDir, t.TempDir(), spaceID, sessionapi.Permissions{Read: true, Write: true, Admin: true}, sessionapi.Errors{Closed: errors.New("closed"), NotFound: errors.New("not found"), Unauthorized: errors.New("unauthorized"), Conflict: errors.New("conflict")})
	defer sess.Close()
	if _, err := sess.AddNode(ctx, sessionapi.AddNodeInput{Content: "manifest-backed"}); err != nil {
...
func TestFileSessionNodeTimestamps(t *testing.T) {
	...
	node, err := sess.AddNode(ctx, sessionapi.AddNodeInput{Content: "created"})
	if err != nil {
	...
	updated, err := sess.UpdateNode(ctx, sessionapi.UpdateNodeInput{ID: node.ID, Content: "updated", Props: node.Props})
	if err != nil {
...
func TestFileSessionMoveSubtreeWithinSameParentReorders(t *testing.T) {
	...
	if _, err := sess.ApplyGraph(ctx, sessionapi.ApplyGraphInput{
		AddNodes: []sessionapi.AddNodeInput{{ID: &rootID}, {ID: &aID}, {ID: &bID}, {ID: &cID}},
...
func TestFileSessionMoveRootNodeUnderParent(t *testing.T) {
	...
	if _, err := sess.ApplyGraph(ctx, sessionapi.ApplyGraphInput{AddNodes: []sessionapi.AddNodeInput{{ID: &parentID}, {ID: &rootID}}}); err != nil {
		t.Fatalf("build graph failed: %v", err)
	}
...
func TestFileSessionMoveSubtreeRejectsInvalidMoves(t *testing.T) {
	...
	if _, err := sess.ApplyGraph(ctx, sessionapi.ApplyGraphInput{
		AddNodes: []sessionapi.AddNodeInput{{ID: &rootID}, {ID: &aID}, {ID: &bID}},
		AddEdges: []sessionapi.AddEdgeInput{containsInput(rootID, aID, 0), containsInput(aID, bID, 0)},
	}); err != nil {
		t.Fatalf("build graph failed: %v", err)
	}
	if _, err := sess.MoveSubtree(ctx, sessionapi.MoveSubtreeInput{NodeID: aID, NewParentID: bID}); !errors.Is(err, errInvalidInput) {
		t.Fatalf("expected cycle move invalid input, got %v", err)
	}
	if _, err := sess.MoveSubtree(ctx, sessionapi.MoveSubtreeInput{NodeID: aID, NewParentID: aID}); !errors.Is(err, errInvalidInput) {
	...
	badOrder := -1
	if _, err := sess.MoveSubtree(ctx, sessionapi.MoveSubtreeInput{NodeID: aID, NewParentID: rootID, Order: &badOrder}); !errors.Is(err, errInvalidInput) {
...
func TestFileSessionReorderChildrenRequiresCompleteListAndQueryUsesOrder(t *testing.T) {
	...
	if _, err := sess.ReorderChildren(ctx, sessionapi.ReorderChildrenInput{ParentID: rootID, ChildIDs: []graph.NodeID{cID, aID}}); !errors.Is(err, errInvalidInput) {
		t.Fatalf("expected missing child invalid input, got %v", err)
	}
	if _, err := sess.ReorderChildren(ctx, sessionapi.ReorderChildrenInput{ParentID: rootID, ChildIDs: []graph.NodeID{cID, aID, aID}}); !errors.Is(err, errInvalidInput) {
		t.Fatalf("expected duplicate child invalid input, got %v", err)
	}
	extraID := nodeID()
	if _, err := sess.ReorderChildren(ctx, sessionapi.ReorderChildrenInput{ParentID: rootID, ChildIDs: []graph.NodeID{cID, aID, extraID}}); !errors.Is(err, errInvalidInput) {
...
func BenchmarkUpdateNodeAndCreateSibling(b *testing.B) {
	ctx := context.Background()
	sess := newHierarchyBenchmarkSession(b)
	rootID, entryID := nodeID(), nodeID()
	if _, err := sess.ApplyGraph(ctx, sessionapi.ApplyGraphInput{
		AddNodes: []sessionapi.AddNodeInput{{ID: &rootID}, {ID: &entryID, Content: "entry"}},
...
func newHierarchyTestSession(t *testing.T) sessionapi.Session {
	t.Helper()
	spaceID := domainspace.SpaceID(uuid.New())
	graphsDir := t.TempDir()
	prepareSpaceDir(t, graphsDir, spaceID)
	return New(graphsDir, t.TempDir(), spaceID, sessionapi.Permissions{Read: true, Write: true, Admin: true}, sessionapi.Errors{Closed: errors.New("closed"), NotFound: errors.New("not found"), Unauthorized: errors.New("unauthorized"), Conflict: errors.New("conflict")})
}

func newHierarchyBenchmarkSession(b *testing.B) sessionapi.Session {
	b.Helper()
	spaceID := domainspace.SpaceID(uuid.New())
	graphsDir := b.TempDir()
	prepareBenchmarkSpaceDir(b, graphsDir, spaceID)
	sess := New(graphsDir, b.TempDir(), spaceID, sessionapi.Permissions{Read: true, Write: true, Admin: true}, sessionapi.Errors{Closed: errors.New("closed"), NotFound: errors.New("not found"), Unauthorized: errors.New("unauthorized"), Conflict: errors.New("conflict")})
