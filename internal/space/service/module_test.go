...
func TestModuleWALCreateSpaceAppendsAndApplies(t *testing.T) {
	...
	rt := &daemonruntime.Runtime{Config: config.Config{DataDir: dataDir}, LoggerValue: slog.Default(), WALValue: walManager, RegistryValue: wal.NewRegistry(), ProgressValue: progress, WaiterValue: waiter}
	m := NewModule()
	if result := m.Init(ctx, rt); !result.OK {
		t.Fatalf("init failed: %v", result.Error)
	}
...
func TestModuleWALRecoveryAppliesCommittedCreateSpace(t *testing.T) {
	...
	rt := &daemonruntime.Runtime{Config: config.Config{DataDir: dataDir}, LoggerValue: slog.Default(), WALValue: walManager, RegistryValue: registry}
	m := NewModule()
	if result := m.Init(ctx, rt); !result.OK {
		t.Fatalf("init failed: %v", result.Error)
	}
