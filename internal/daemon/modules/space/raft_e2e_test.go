...
func TestModuleCreateSpaceWithResultUsesRaftProposalWhenEnabled(t *testing.T) {
	...
	m := NewModule()
	if result := m.Init(ctx, &daemonruntime.Runtime{Config: config.Config{DataDir: t.TempDir(), Cluster: config.ClusterConfig{RaftPartitionCount: 64}}, Logger: slog.Default()}); !result.OK {
