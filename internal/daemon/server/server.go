...
func New(cfg Config, opts ...grpc.ServerOption) (*Server, error) {
	...
	adminv1.RegisterAdminIntelligenceAccessCredentialServiceServer(grpcServer, adminInference)
	adminv1.RegisterAdminIntelligenceAccessGrantServiceServer(grpcServer, adminInference)
	...
	if cfg.AutomationManager != nil {
		adminv1.RegisterAdminAutomationServiceServer(grpcServer, adminapi.NewAdminAutomationService(cfg.AutomationManager, cfg.PrincipalManager).WithDomainResolver(cfg.SpaceManager))
	}
...
func defaultPublicMethods() map[string]bool {
	return map[string]bool{
		...
		clusterpb.ClusterBackendService_ExecuteRaftAutomationRuntimeRead_FullMethodName: true,
