...
func New(cfg Config, opts ...grpc.ServerOption) (*Server, error) {
	...
	baseOptions := []grpc.ServerOption{grpc.ChainUnaryInterceptor(cfg.TokenManager.UnaryInterceptor(publicMethods)), grpc.ChainStreamInterceptor(cfg.TokenManager.StreamInterceptor(publicMethods))}
