...
func initializeExperimentalRaft(ctx context.Context, rt *daemonruntime.Runtime, systemStateMachine func() consensus.StateMachine, partitionStateMachine func(uint32) consensus.StateMachine) error {
	...
	groups, err := consensus.StartMultiGroup(ctx, consensus.MultiGroupOptions{NodeID: consensus.NodeID(cfg.RaftLocalNodeID), PeerNodeIDs: peers, PartitionCount: uint32(cfg.RaftPartitionCount), Transport: transport, StateMachines: factory, ElectionTick: cfg.RaftElectionTick, HeartbeatTick: cfg.RaftHeartbeatTick, StorageDir: storageDir, Encryption: rt.Encryption, DeferPartitionGroups: true, RecoverEmptyStorageRejoin: cfg.RaftEmptyStorageRejoinRecovery})
	if err != nil {
