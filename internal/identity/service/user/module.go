...
func (m *Module) Init(ctx context.Context, host runtime.Host) runtime.InitResult {
	userDir := filepath.Join(host.DataDir(), "users")
	userDirCreated, err := ensureDir(userDir, 0o700)
	if err != nil {
		return runtime.Abort(ModuleName, "filesystem", "failed to create users directory", err)
	}
	if logger := host.Log(); logger != nil {
		logger.Info("user directory ready", "path", userDir, "created", userDirCreated)
	}

	store, storeCreated, err := OpenStore(userDir)
	if err != nil {
		return runtime.Abort(ModuleName, "store", "failed to open user store", err)
	}
	m.store = store
	m.dataDir = host.DataDir()
	if logger := host.Log(); logger != nil {
		logger.Info("user store ready", "path", filepath.Join(userDir, StoreFilename), "created", storeCreated)
	}

	sessionsDir := filepath.Join(userDir, "sessions")
	sessionsDirCreated, err := ensureDir(sessionsDir, 0o700)
	if err != nil {
		return runtime.Abort(ModuleName, "filesystem", "failed to create user sessions directory", err)
	}
	if logger := host.Log(); logger != nil {
		logger.Info("user sessions directory ready", "path", sessionsDir, "created", sessionsDirCreated)
	}
	sessions := storesession.NewManager()
	if err := sessions.Init(ctx, sessionsDir); err != nil {
		return runtime.Abort(ModuleName, "store", "failed to open user session store", err)
	}
	m.sessions = sessions
	...
	if provider, ok := host.(runtime.WALProvider); ok {
		if registry := provider.WALRegistryStore(); registry != nil {
			if err := registry.Register(recordTypeUserPut, wal.ApplierFunc(m.applyUserPut)); err != nil {
				return runtime.Abort(ModuleName, "wal", "register user put WAL applier", err)
			}
		}
	}
	if m.gate == nil {
	...
	if _, ok := host.(runtime.QuiesceRegistrar); ok {
		if err := host.(runtime.QuiesceRegistrar).RegisterQuiesceParticipant(m.gate); err != nil {
			return runtime.Abort(ModuleName, "quiesce", "register user quiesce participant", err)
		}
	}
