...
func TestModuleWALUserMutationsAppendAndApply(t *testing.T) {
	...
	rt := &daemonruntime.Runtime{Config: config.Config{DataDir: dataDir}, LoggerValue: slog.New(slog.NewTextHandler(&logs, nil)), WALValue: walManager, RegistryValue: wal.NewRegistry(), ProgressValue: progress, WaiterValue: wal.NewApplyWaiter()}
	module := NewModule()
...
func initUserModule(t *testing.T, dataDir string) *Module {
	...
	rt := &daemonruntime.Runtime{Config: config.Config{DataDir: dataDir, Mode: "standalone", LogLevel: "debug", LogFormat: "text"}, LoggerValue: logger}
	module := NewModule()
