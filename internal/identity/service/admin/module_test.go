...
func TestModuleWALOperatorMutationsAppendAndApply(t *testing.T) {
	...
	var logs bytes.Buffer
	rt := &daemonruntime.Runtime{Config: config.Config{DataDir: dataDir, Mode: "mesh"}, LoggerValue: slog.New(slog.NewTextHandler(&logs, nil)), WALValue: walManager, RegistryValue: wal.NewRegistry(), ProgressValue: progress, WaiterValue: wal.NewApplyWaiter()}
	module := NewModule()
