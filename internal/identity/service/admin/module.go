...
func (m *Module) Init(ctx context.Context, host runtime.Host) runtime.InitResult {
	adminDir := filepath.Join(host.DataDir(), "admins")
	adminDirCreated, err := ensureDir(adminDir, 0o700)
	if err != nil {
		return runtime.Abort(ModuleName, "filesystem", "failed to create admins directory", err)
	}
	if logger := host.Log(); logger != nil {
		logger.Info("admin directory ready", "path", adminDir, "created", adminDirCreated)
	}

	store, storeCreated, err := OpenStore(adminDir)
	if err != nil {
		return runtime.Abort(ModuleName, "store", "failed to open admin store", err)
	}
	m.store = store
	...
	if err != nil {
		return runtime.Abort(ModuleName, "filesystem", "failed to create admin sessions directory", err)
	}
	if logger := host.Log(); logger != nil {
		logger.Info("admin sessions directory ready", "path", sessionsDir, "created", sessionsDirCreated)
	}
	sessions := storesession.NewManager()
	if err := sessions.Init(ctx, sessionsDir); err != nil {
		return runtime.Abort(ModuleName, "store", "failed to open admin session store", err)
	}
	m.sessions = sessions
	...
	if provider, ok := host.(runtime.WALProvider); ok {
		if registry := provider.WALRegistryStore(); registry != nil {
			if err := registry.Register(recordTypeAdminPut, wal.ApplierFunc(m.applyAdminPut)); err != nil {
				return runtime.Abort(ModuleName, "wal", "register admin put WAL applier", err)
			}
		}
	}
	if m.gate == nil {
	...
	if _, ok := host.(runtime.QuiesceRegistrar); ok {
		if err := host.(runtime.QuiesceRegistrar).RegisterQuiesceParticipant(m.gate); err != nil {
			return runtime.Abort(ModuleName, "quiesce", "register admin quiesce participant", err)
		}
	}
	if logger := host.Log(); logger != nil {
		logger.Info("admin store ready", "path", filepath.Join(adminDir, StoreFilename), "created", storeCreated)
	}

	admins, err := store.List(ctx)
	if err != nil {
		return runtime.Abort(ModuleName, "store", "failed to list admins", err)
	}
	if hostMode(host) == "standalone" && len(admins) == 0 {
		username, password, generated, err := bootstrapAdminCredentials(hostBootstrapAdminUsername(host), hostBootstrapAdminPassword(host))
		if err != nil {
			return runtime.Abort(ModuleName, "security", "failed to resolve default admin credentials", err)
		}
		hash, err := HashPassword(password)
		if err != nil {
			return runtime.Abort(ModuleName, "security", "failed to hash default admin password", err)
		}
		now := time.Now().UTC()
		...
		if err := store.Create(ctx, admin); err != nil {
			if errors.Is(err, ErrDuplicateAdmin) {
				return runtime.Continue(ModuleName, "store", "default admin already exists", err)
			}
			return runtime.Abort(ModuleName, "store", "failed to create default admin", err)
		}
		if logger := host.Log(); logger != nil {
			if generated {
				logger.Warn("default standalone admin created; change this password immediately", "username", admin.Username, "password", password, "change_password_required", true)
			} else {
				logger.Warn("default standalone admin created from configured bootstrap credentials", "username", admin.Username, "password_configured", true, "change_password_required", true)
			}
		}
	}
	if hostMode(host) == "standalone" {
		if err := m.ensureStandaloneSystemAdmin(ctx, host.Log()); err != nil {
			return runtime.Abort(ModuleName, "store", "failed to ensure standalone system admin", err)
		}
	}
	return runtime.OK(ModuleName)
}

func hostMode(host runtime.Host) string {
	return stringHostConfigField(host, "Mode")
}
