...
func NewAdminBackupPolicySetCommand(a *app.App) *cobra.Command {
	var enabled, disabled, includeLogs, allowReads bool
...
func printBackupPolicy(a *app.App, policy *adminv1.BackupPolicy) error {
	...
	text := fmt.Sprintf("backup policy:\n  enabled: %t\n  dir: %s\n  interval: %s\n  keep: %d\n  include_logs: %t\n  compression: %s\n  quiesce_timeout: %s\n  backup_timeout: %s\n  retry_after: %s\n  history_limit: %d\n  allow_reads_during_backup: %t\n", policy.GetEnabled(), policy.GetBackupDir(), formatSeconds(policy.GetIntervalSeconds()), policy.GetRetentionCount(), policy.GetIncludeLogs(), policy.GetCompression(), formatSeconds(policy.GetQuiesceDrainTimeoutSeconds()), formatSeconds(policy.GetBackupTimeoutSeconds()), formatSeconds(policy.GetRetryAfterSeconds()), policy.GetStatusHistoryLimit(), policy.GetAllowReadsDuringBackup())
