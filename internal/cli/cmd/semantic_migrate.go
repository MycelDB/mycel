...
func newSemanticMigrateLegacyEmbeddingsCommand(a *app.App) *cobra.Command {
	...
	cmd.Flags().StringVar(&ownerUserID, "owner-user-id", "", "legacy embedding owner user UUID (daemon mode defaults to the space owner)")
	cmd.Flags().IntVar(&limit, "limit", 0, "maximum profiles to migrate in daemon mode (0 for all)")
	...
	cmd.Flags().BoolVar(&dryRun, "dry-run", false, "validate and report migratable legacy profiles without writing resources (daemon mode)")
	return cmd
}
