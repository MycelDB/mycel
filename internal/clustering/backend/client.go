...
func (c Client) GetClusterView(ctx context.Context, addr string) (model.Snapshot, error) {
	...
	defer conn.Close()
	res, err := clusterpb.NewClusterBackendServiceClient(conn).GetClusterView(ctx, &clusterpb.GetClusterViewRequest{ProtocolVersion: clusterpb.ClusterProtocolVersion_CLUSTER_PROTOCOL_VERSION_V1})
	if err != nil {
		return model.Snapshot{}, err
	}
