...
func (a BackendAdapter) RegisterNode(ctx context.Context, addr string, in RegisterNodeInput) (RegisterNodeResult, error) {
	res, err := a.Client.RegisterNode(ctx, addr, backend.RegisterNodeInput{Identity: in.Identity, State: in.State, KnownPeers: in.KnownPeers, NodePublicKeyFingerprint: in.NodePublicKeyFingerprint})
	if err != nil {
