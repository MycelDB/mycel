...
func (m *Module) proposeSemanticRaftCommand(ctx context.Context, cmd consensus.RaftCommand) error {
	_, err := m.proposeSemanticRaftCommandWithResult(ctx, cmd)
	return err
}

func (m *Module) proposeSemanticRaftCommandWithResult(ctx context.Context, cmd consensus.RaftCommand) (consensus.ProposalResult, error) {
	if m.raftGroups == nil {
		return consensus.ProposalResult{}, status.Error(codes.Unavailable, "semantic raft groups are not configured")
	}
	group, ok := m.raftGroups.Group(consensus.PartitionGroupID(cmd.PartitionID))
	if !ok || group == nil {
		return consensus.ProposalResult{}, status.Errorf(codes.Unavailable, "semantic raft partition group %d is not available", cmd.PartitionID)
	}
	if group.Leader() == 0 {
		return consensus.ProposalResult{}, status.Errorf(codes.Unavailable, "semantic raft partition group %d has no leader", cmd.PartitionID)
	}
	result, err := group.Propose(ctx, cmd)
	if err != nil {
		return consensus.ProposalResult{}, status.Errorf(codes.Unavailable, "semantic raft proposal for partition %d failed: %v", cmd.PartitionID, err)
	}
	return result, nil
}
