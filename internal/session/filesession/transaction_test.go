...
func TestFileSessionTransactionPhase2CommitUnsupportedAndCallbackRollback(t *testing.T) {
	...
	if err := tx.Commit(ctx); !errors.Is(err, sessionapi.ErrTransactionsUnsupported) {
		t.Fatalf("expected ErrTransactionsUnsupported from commit, got %v", err)
	}
	if err := tx.Rollback(ctx); !errors.Is(err, sessionapi.ErrTransactionClosed) {
		t.Fatalf("expected closed tx after commit attempt, got %v", err)
	}
