...
The `session_renewal` branch includes early durable refresh-session building blocks: `domain/auth`, refresh-token generation/hash helpers, `store/session`, and refresh-session configuration keys under `auth.refresh_*`. These are not yet wired into public login/refresh APIs.
