...
A successful manual backup updates status. For interval schedules, it pushes the next scheduled run out by the configured interval; daily/weekly wall-clock schedules are not shifted by manual backups. Only one backup runs at a time; concurrent triggers return an already-running conflict (`codes.Aborted` from the Admin API), which operators may retry after the active backup finishes.
