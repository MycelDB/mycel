...
| `daemonsemantic.RaftStateMachine` | System | `semantic.global.mutation.v1` | Global semantic config stores. | System-scoped JSON snapshot/restore exists for global semantic metadata. Vector index payloads remain derived/rebuildable. | Semantic raft applied command IDs are included by system command prefix. | `snapshot-capable-initial` plus `derived/rebuildable` vector state | Authoritative semantic config metadata; derived-state marker for vector indexes. Standalone inference owns usage telemetry. | Need stronger freshness/degraded diagnostics after restore before production compaction. |
