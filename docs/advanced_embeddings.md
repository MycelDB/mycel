...
This section describes how the advanced embedding model should map onto Mycel's file-backed storage.

The current Mycel store already uses a single data root with JSON metadata and append-only per-space graph/vector segment files. The proposed advanced embedding model should preserve that style:

- global/deployment definitions live under `meta/`
- user/space/domain metadata lives under `meta/`
- graph content remains in append-only graph segments under `graphs/<space_id>/`
- vector records remain append-only under the relevant space
- operational semantic-index state is explicit and recoverable
...
This is intentionally still file-backed and simple. Later implementations can split large JSON files into per-record files or append-only metadata logs if needed.

### Global Inference Definitions

Global/deployment-level inference definitions should be stored under:
...
- `packages.json`: records installed inference packages, versions, checksums, and source paths/URLs.
- `runtimes.json`: provisioned runtime definitions such as OpenAI public API, local Ollama, or an enterprise gateway.
- `models.json`: model metadata such as model names, dimensions, operations, modalities, and vector-space keys.
- `vector_stores.json`: configured vector store backends such as `mycel-file`, Qdrant, pgvector, or custom remote stores.

These are not graph content and are not owned by one space. They describe what this Mycel installation knows how to call or store.

### Secrets, Credentials, and Grants

Credentials should be split from secret material:
...
- `secrets.json` stores encrypted secret payloads or references to external secret managers.
- `credentials.json` stores credential metadata: owner, runtime ID, auth type, status, secret reference, default marker.
- `grants.json` stores atomic credential grants: one credential, one processing scope, allowed operations, optional runtime/model constraints, priority/default metadata.

A credential grant for a domain might be stored conceptually as:
...
Inference/content policies should be stored under:

```text
meta/policies/inference_policies.json
```

Policies reference scopes rather than being embedded directly into graph nodes. This keeps policy management auditable and avoids rewriting graph segment records when policies change.
...
At runtime, Mycel rebuilds or maintains in-memory policy indexes keyed by:

- space ID
- domain ID
- semantic index ID
- node ID/subtree root

This makes policy evaluation efficient during writes, backfill, refresh, and query planning.

### Per-Space Semantic Index Metadata

Semantic indexes should live with the space because they are views over graph content in that space/domain:
...
`indexes.json` contains definitions:
...
`index_state.json` stores operational state:
...
### Dirty Queue / Maintenance Work

When a block is created or changed under a policy/index that recommends embeddings, Mycel should not call an inference runtime synchronously. Instead it should persist dirty work:
...
The dirty queue should coalesce by:

```text
semantic_index_id + target_node_id
```

For a `self` source mode, the target node is usually the changed node. For a `subtree` source mode, the target node is usually the semantic root selected by the index, such as a journal/page node.

### Policy Decisions

Policy evaluations that affect persisted embedding work can be recorded under:
...
This store can be bounded or compacted. Not every transient query decision must be persisted forever, but decisions that cause backfill/refresh skips or embedding records should be inspectable.

### Per-Index Vector Records

For embedded vector storage, each semantic index can have its own append-only vector records:
...
- vector space key
- dimensions
- vector payload or vector reference
- created timestamp

For external vector stores, Mycel may not store the vector payload locally. Instead it should store references and synchronization metadata:
...
6. the maintainer resolves credentials from `meta/credentials/*.json`
7. generated records are appended to the semantic index vector store
8. state is updated in `index_state.json`

This keeps graph writes fast and durable while making semantic maintenance resumable after process restarts.

### Compaction and Recovery

The initial implementation can use JSON metadata files plus append-only vector records. On startup, Mycel can rebuild in-memory indexes from:

- graph segment records
- semantic index definitions
- dirty queue records
- policy records
- credential grants
- vector record manifests/segments

Later compaction should address:

- old stale vector records
