...
- `make test-compose-cluster` — destructive local compose validation for fresh bootstrap, real pod-to-pod graph write/read/query/consistency, restart data-plane stability, and persisted identity stability. Uses the Mycel-owned fixture at `tests/compose/cluster/compose.yml` and requires Docker. The target supplies a default compose-only `MYCELD_CLUSTER_BACKEND_AUTH_TOKEN` if the environment does not set one.
