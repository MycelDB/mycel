...
| [Raft disruption test](raft-disruption-test-harness.md) | `make test-k3s-raft-disruption-smoke`, `make test-k3s-raft-disruption`, `make test-k3s-raft-disruption-edges`, `make test-k3s-raft-restart-soak`, `make test-k3s-raft-restart-hard-soak`, or `go run ./cmd/mycel-raft-disrupttest ...` | Reusable disposable k3d/K3s pod-restart pressure test with smoke, small, edge, multi-space, medium, soak, and restart/write soak variations. |
