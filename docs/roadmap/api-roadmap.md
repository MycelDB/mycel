...
| General index pushdown | Use graph/metadata/semantic indexes for all compatible predicates/traversals instead of full in-memory scans. | Very High | Very High | N | Accepted indexed node, ordered, adjacency, and root-subtree paths exist; broader predicate/traversal/text/semantic pushdown is still missing. |
