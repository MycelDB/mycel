...
STATE_FILE="${MYCEL_K3S_DATA_PLANE_STATE:-${MYCEL_DATA_PLANE_STATE:-}}"
CREATE_IF_MISSING="${MYCEL_DATA_PLANE_CREATE_IF_MISSING:-true}"
TIMEOUT_SECONDS="${MYCEL_K3S_DATA_PLANE_TIMEOUT:-180}"
SLEEP_SECONDS="${MYCEL_K3S_DATA_PLANE_INTERVAL:-3}"
CLI_TIMEOUT_SECONDS="${MYCEL_K3S_CLI_TIMEOUT:-25}"
...
create_data() {
  ...
  for candidate in "${PODS[@]}"; do
    ...
    echo "Graph write through $candidate did not reach the current partition leader; trying next pod" >&2
  done
  if [[ "$wrote" != "true" ]]; then
    echo "failed to write Phase G5 graph data through any configured pod" >&2
    return 1
  fi
...
validate_query_response() {
  local pod="$1" raw="$2"
  RAW_JSON="$raw" python3 - "$pod" "$PARENT_NODE_ID" "$MARKER" <<'PY'
import json, os, sys
pod, node_id, marker = sys.argv[1:4]
data = json.loads(os.environ["RAW_JSON"])
rows = data.get("rows") or []
if len(rows) != 1:
    raise SystemExit(f"{pod}: expected one query row, got {len(rows)}")
meta = data.get("read_metadata") or {}
if meta.get("consistency") != "strong" or bool(meta.get("stale")):
    raise SystemExit(f"{pod}: expected strong non-stale read metadata, got {meta}")
field = (rows[0].get("fields") or {}).get("node") or {}
node = (((field.get("Value") or {}).get("Node")) or field.get("node") or {})
if node.get("node_id") != node_id:
    raise SystemExit(f"{pod}: query returned node {node.get('node_id')} expected {node_id}")
props = node.get("properties") or {}
if props.get("marker") != marker:
    raise SystemExit(f"{pod}: query marker mismatch: {props}")
PY
}

validate_node_response() {
  local pod="$1" raw="$2" want="$3"
  RAW_JSON="$raw" python3 - "$pod" "$want" <<'PY'
import json, os, sys
pod, want = sys.argv[1:3]
data = json.loads(os.environ["RAW_JSON"])
if data.get("node_id") != want:
    raise SystemExit(f"{pod}: node_id {data.get('node_id')} expected {want}")
PY
}

validate_edge_response() {
  local pod="$1" raw="$2"
  RAW_JSON="$raw" python3 - "$pod" "$EDGE_ID" "$PARENT_NODE_ID" "$CHILD_NODE_ID" <<'PY'
import json, os, sys
pod, edge_id, parent, child = sys.argv[1:5]
data = json.loads(os.environ["RAW_JSON"])
if data.get("edge_id") != edge_id or data.get("from_node_id") != parent or data.get("to_node_id") != child:
    raise SystemExit(f"{pod}: unexpected edge payload {data}")
PY
}

validate_pod_once() {
  local pod="$1" raw session_id tx_id
  ...
  tx_id="$(printf '%s\n' "$raw" | json_get 'transaction_id')"
  raw="$(user_cli "$pod" graph node get "$PARENT_NODE_ID" --transaction-id "$tx_id")"
  validate_node_response "$pod" "$raw" "$PARENT_NODE_ID"
  raw="$(user_cli "$pod" graph node get "$CHILD_NODE_ID" --transaction-id "$tx_id")"
  validate_node_response "$pod" "$raw" "$CHILD_NODE_ID"
  raw="$(user_cli "$pod" graph edge get "$EDGE_ID" --transaction-id "$tx_id")"
  validate_edge_response "$pod" "$raw"
  raw="$(user_cli "$pod" query nodes --transaction-id "$tx_id" --label G5Parent --property-equals "marker=$MARKER" --limit 5)"
  validate_query_response "$pod" "$raw"
...
validate_consistency_report_once() {
  local pod="$1" raw
  raw="$(admin_cli "$pod" cluster consistency-report --space-id "$SPACE_ID" --domain-id "$DOMAIN_ID")"
  RAW_JSON="$raw" python3 - "$pod" <<'PY'
import json, os, sys
pod = sys.argv[1]
data = json.loads(os.environ["RAW_JSON"])
status = data.get("status")
if status != "consistent":
    raise SystemExit(f"{pod}: expected consistency report status=consistent, got {status}: {data}")
if len(data.get("replicas") or []) < 1:
    raise SystemExit(f"{pod}: consistency report has no replicas")
PY
}
...
validate_once() {
  ...
  for pod in "${PODS[@]}"; do
    pod="$(trim "$pod")"
    [[ -n "$pod" ]] || continue
    validate_pod_once "$pod"
  done
  validate_consistency_report_once "$(pod_at 0)"
}

if [[ -s "$STATE_FILE" ]]; then
  load_state
elif [[ "$CREATE_IF_MISSING" == "true" ]]; then
  create_data "$(pod_at 0)"
else
...
while (( SECONDS <= deadline )); do
  if output="$(validate_once 2>&1)"; then
    echo "K3s cluster data-plane validation passed"
    echo "state_file=$STATE_FILE"
    echo "space_id=$SPACE_ID domain_id=$DOMAIN_ID marker=$MARKER committed_revision=${COMMITTED_REVISION:-0}"
    exit 0
  fi
  last_error="$output"
  echo "Waiting for K3s cluster data-plane validation: $last_error" >&2
  sleep "$SLEEP_SECONDS"
done

echo "K3s cluster data-plane validation failed after ${TIMEOUT_SECONDS}s" >&2
echo "$last_error" >&2
for pod in "${PODS[@]}"; do
  pod="$(trim "$pod")"
  [[ -n "$pod" ]] || continue
  echo "--- logs: $pod ---" >&2
  kubectl -n "$NAMESPACE" logs --tail=80 "$pod" >&2 || true
done
exit 1
